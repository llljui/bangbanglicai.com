...
class DealAction extends CommonAction{
	...
	public function add()
	{
        ...
        //用户列表
        $user_list = M('User')->field('user_name, real_name, mobile, id')->where('is_effect = 1 AND is_delete = 0')->limit(10000)->findAll();
...
//								$vo['user_bid_score_fee']= $vol_list[$ks]['6'];
	...
	public function ajax_get_user_list(){
		...
		$search_data =$_REQUEST['user_data'];
		if(!$search_data){
			ajax_return($return);
			exit();
		}
        ...
        if (is_numeric($search_data)) {
            $where .= ' AND mobile LIKE "%' . $search_data . '%"';
        } else {
            $where .= ' AND (real_name LIKE "%' . $search_data . '%" OR user_name LIKE "%' . $search_data . '%")';
        }
        $list = M('User')->field('id, real_name, user_name, mobile')->where($where)->findAll();
        if ($list) {
            $return['status'] = 1;
            $return['data'] = $list;
        }
